...
class UserServiceApplicationTests {
    ...
    void getAll_returnsTwoElementList() throws Exception {
        List<UserDto> list = Arrays.asList(new UserDto(1L, "TestOne", "test1@test.com", 36, LocalDateTime.now()), new UserDto(2L, "TestTwo", "test2@test.com", 37, LocalDateTime.now()));

        when(userService.getAll()).thenReturn(list);

        mockMvc.perform(get("/api/users")).andExpect(status().isOk()).andExpect(jsonPath("$[0].name").value("TestOne")).andExpect(jsonPath("$[1].name").value("TestTwo"));
    }
    ...
    void getById_returnsUserDto() throws Exception {
        ...
        mockMvc.perform(get("/api/users/1")).andExpect(status().isOk()).andExpect(jsonPath("$.name").value("TestOne"));
    }
    ...
    void create_shouldCreateUser() throws Exception {
        ...
        mockMvc.perform(post("/api/users").contentType(MediaType.APPLICATION_JSON).content("""
                ...
                """)).andExpect(status().isCreated()).andExpect(jsonPath("$.name").value("TestOne"));
    }
    ...
    void update_shouldUpdateUser() throws Exception {
        ...
        mockMvc.perform(put("/api/users/1").contentType(MediaType.APPLICATION_JSON).content("""
                ...
                """)).andExpect(status().isOk()).andExpect(jsonPath("$.name").value("TestOne"));
    }
